...
def train(args, model, device, x, y, optimizer, criterion, task_id_nominal, consolidated_masks):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        optimizer.zero_grad()
        feature, output = model(data, task_id_nominal, mask=None, mode="train")
    ...
    with torch.no_grad():
        ...
        for i in range(0,len(r),args.batch_size_test):
            ...
            if comp_flag:
                ...
                feature, output = model(data, task_id_nominal, mask=dec_task_mask, mode=mode)
            else:
                feature, output = model(data, task_id_nominal, mask=curr_task_masks, mode=mode)
            loss = criterion(output, target)
