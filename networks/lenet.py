...
class SubnetLeNet(nn.Module):
    ...
    def get_masks(self, task_id):
        ...
        for name, module in self.named_modules():
            ...
            if isinstance(module, SubnetLinear) or isinstance(module, SubnetConv2d):
                print(name)
                task_mask[name + '.weight'] = (module.weight_mask.detach().clone() > 0.5).type(torch.uint8)

                if getattr(module, 'bias') is not None:
                    task_mask[name + '.bias'] = (module.bias_mask.detach().clone() > 0.5).type(torch.uint8)
                else:
...
class GambelLeNet(nn.Module):
    def __init__(self,taskcla, sparsity):
        super(GambelLeNet, self).__init__()
        self.in_channel =[]
        self.conv1 = GambelConv2d(3, 20, 5, bias=False, padding=2)

        s=compute_conv_output_size(32,5,1,2)
        s=compute_conv_output_size(s,3,2,1)
        self.in_channel.append(3)    
        self.conv2 = GambelConv2d(20, 50, 5, bias=False, padding=2)
        ...
        self.fc1 = GambelLinear(50*self.smid*self.smid, 800,  bias=False)
        self.fc2 = GambelLinear(800, 500, bias=False)
        ...
        self.none_masks = {}
        for name, module in self.named_modules():
            if isinstance(module, GambelLinear) or isinstance(module, GambelConv2d):
                self.none_masks[name + '.weight'] = None
                self.none_masks[name + '.bias'] = None
    ...
    def get_masks(self, task_id):
        ...
        for name, module in self.named_modules():
            ...
            if isinstance(module, GambelLinear) or isinstance(module, GambelConv2d):
                ...
                task_mask[name + '.weight'] = (module.mask.detach().clone() > 0.5).type(torch.uint8)
